    ...
    internal class AuthProcessor
    {
        ...
        internal bool Auth(string user, string pass, out string reason)
        {
            try
            {
                ...
                //AuthClientHandler.Instance.Write(_hardwareId, HardwareId.Value().ToByte());
                //packet = AuthClientHandler.Instance.GetNextPacket();
...
#else
                AuthClientHandler.Instance.Write(_md5String, Program.GetMD5AsBase64(Assembly.GetExecutingAssembly().Location).ToByte());
#endif
